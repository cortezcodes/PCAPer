...
from protocols.ICMPPacket import ICMPPacket
...
def tcp_templates_prompt():
    '''
    Displays the tcp template table and provides prompts for pcap creation from templat
    '''
    clear()
    title="TCP Templates"
...
def icmp_template_prompt():
    '''
    Displays the ICMP template table and provides prompts for pcap creation from templat
    '''
    clear()
    title="ICMP Templates"
    columns = ICMPPacket.get_parameters_list()
    columns.insert(0, "#")


    icmp_templates: List[PacketTemplate] = get_templates("icmp")
    table = create_table(type="icmp", title=title, columns=columns, templates=icmp_templates)
...
def create_icmp_prompt():
    ...
    while not confirmed:
        ...
        src_ip = typer.prompt("Source IP", default="1.1.1.1")
        dst_ip = typer.prompt("Destination IP", default="2.2.2.2")
        message_type = typer.prompt("Message Type", default=8)
        code = typer.prompt("Code", default=0)
        seq = typer.prompt("sequence", default=0)
        ident = typer.prompt("identifer", default=1)
        filepath = typer.prompt("PCAP filepath", default="./pcap_files/icmp_output.pcap")

        clear()
        icmp_packet:ICMPPacket = ICMPPacket(src_ip=src_ip, dst_ip=dst_ip, message_type=message_type,
                                            code=code, seq=seq, ident=ident)
        
        console.print(str(icmp_packet)+f"\nfilepath: {filepath}\n\n")
        confirmed = typer.confirm("Confirm")
    ...
    make_template: bool = typer.confirm("Would you like to make this packet into a template?")    
    icmp_packet.generate_packet(filepath)

    clear()
    console.print(f"pcap file created")
    
    if make_template:
        create_packet_template_prompter(icmp_packet)


def create_packet_template_prompter(packet: UDPPacket | TCPPacket | ICMPPacket):
    '''
